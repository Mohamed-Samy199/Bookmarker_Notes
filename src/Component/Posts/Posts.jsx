...
const Posts = () => {
    ...
    // display data at model
    const displayHandle = (i) => {
        const data = filteredPosts.find((post) => post.id === i);
        setModalData(data);
        setModalOpen(true);
    };
        ...
        <div className="posts my-5">
            <Modal size="lg" isOpen={modalOpen} toggle={() => setModalOpen(!modalOpen)}>
                <ModalHeader toggle={() => setModalOpen(!modalOpen)}>
                    Note <span className='text-danger'>...</span><i className="fa-solid fa-pencil fs-3 text-danger" style={{transform: "translateY(-5px)"}}></i>
                </ModalHeader>
                <ModalBody>
                    <Row>
                        <Col lg={12}>
                            {modalData && (
                                <Fragment>
                                    ...
                                    <p className='text-muted'>Description: {modalData.desc}</p>
                                </Fragment>
                            )}
                        </Col>
                    </Row>
                        ...
                        <tbody id="tbody">
                            {filteredPosts.map((post, idx) => (
                                <tr key={idx}>
                                    ...
                                    <td className="ps-0">
                                        <button onClick={() => displayHandle(post.id)}>
